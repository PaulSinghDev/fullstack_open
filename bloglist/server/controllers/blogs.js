...
blogsRouter.get('/', async (req, res) => {
  blogs = await Blog.find({})
    ...
    .populate('comments', { date: 1, comment: 1, author: 1, id: 1 })
    .sort({ likes: 'desc' })
...
blogsRouter.delete('/:id', async (req, res) => {
  ...
  if (!blog) {
    return res
      .status(404)
      .json({ error: 'No blog post found with the provided ID' })
  }

  if (blog.user.toString() !== decodedToken.id.toString()) {
    return res.status(401).json({
      error:
        'Permission denied. You do not have the permission to delete this post.',
    })
  }
...
blogsRouter.post('/:id/comment', async (req, res) => {
  const { comment, author = null } = req.body
  const blog = await Blog.findById(req.params.id)
  ...
  if (author) {
    ...
    user.comments = user.comments.concat(commentObj._id)
    commentObj.author = user.id
    await user.save()
  }
  const dbComment = await commentObj.save()

  blog.comments = blog.comments.concat(dbComment._id)
  await blog.save()
